...
let emailex = document.querySelector('.popup__body h2');
if (t == null) {
  ...
  btn.addEventListener("click", function () {
    ...
    let checkemail = checkemail2(emailinput);
    ...
    if (checkUsername == true && checkPassword == true && checkCFPassword == true && checkemail == true ) {
      newUser = {
        email : emailinput,
...
function checkemail2(useremail)  {
if (useremail == "") {
  ...
  let listUser = JSON.parse(localStorage.getItem("listUser"));
  for (let i = 0; i < listUser.length; i++) {
    if (useremail == listUser[i].email) {
     emailex.innerHTML="email existed"
      toggle()
      return false;
    }
  }
